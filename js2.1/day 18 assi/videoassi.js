...
function rendervideo(id){
    ...
    // if u click the thumbnail they should get border
    // $(`card${id}`) or $('#card'+id)
var data = videoPlaySectionData[id-1]
playerwrapper.innerHTML=`<iframe id="video-player" src="https://player.vimeo.com/video/${data.vimeoId}" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
...
/**
 * if u click thumbnail those video should play on left .use on click event.
 * so put active calss name on first image 
 * ${i==0?"active":""}  if u click on first thumbnail put active,when the web paage is loading we should see the first thumnail,
 * in this fn it accepts id
 * 
 * in i frame link u acn see the id and last tht is the vimemoid 
 * data.vimeoid ,so which ever vieo is clicked
 * 
 * 
 * 
 * infor each loop
 * playlistData.forEach(item => {
    playlistwrapper.innerHTML += `
        <div id="card${item.id}" class="playlist-card">
...
//   here we are loading the right side using dynamically. so if u use += it will dispaly all ,so use`` to access all variables and fun
//here instead of id=card1-5,use id from var prodlistwrapper
